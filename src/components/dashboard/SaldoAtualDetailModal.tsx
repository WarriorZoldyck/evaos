...
export function SaldoAtualDetailModal({
  ...
  const openStatement = (target: StatementTarget) => {
    onOpenChange(false);
    // Defer to let the first Dialog unmount cleanly before opening the next
    setTimeout(() => setStatementTarget(target), 0);
  };
                  ...
                  {bankAccounts.map((a) => {
                    ...
                    return (
                      <button
                        key={a.id}
                        onClick={() =>
                          openStatement({ id: a.id, type: "bank", name: a.name, initialBalance: initial })
                        }
                  ...
                  {wallets.map((w) => {
                    ...
                    return (
                      <button
                        key={w.id}
                        onClick={() =>
                          openStatement({ id: w.id, type: "wallet", name: w.name, initialBalance: initial })
                        }
                        className="w-full flex items-center justify-between gap-3 px-4 py-3 border-b border-border last:border-b-0 hover:bg-muted/50 transition-colors text-left"
                      >
                        <div className="min-w-0">
