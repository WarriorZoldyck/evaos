    ...
    if (open) {
      ...
      if (editTransaction) {
        ...
        form.reset({
          ...
          is_installment: !editTransaction.installment_number && (editTransaction.installments ?? 1) > 1,
          installments_count: (editTransaction.installments ?? 1) > 1 ? (editTransaction.installments as number) : 2,
      ...
      <DialogContent key={editTransaction?.id ?? "new"} className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        >
          <TabsList className="grid w-full grid-cols-3">
            ...
            <TabsTrigger
              value="transferencia"
              disabled={isEditing && !!editTransaction?.transfer_id}
            >
              Transf. entre Contas
            </TabsTrigger>
          </TabsList>
